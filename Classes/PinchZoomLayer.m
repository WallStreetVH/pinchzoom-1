...
static const unsigned int WINDOW_WIDTH = 320;
...
- (void)handlePitchZoom:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ((newScale >= minScale) && (newScale <= maxScale)) {
        ...
        // take new center point location (ScrollableView space) and calculate difference
        CGPoint newCenterPointInParentNodeSpace = [self convertPoint:centerPoint fromNode:contentNode];
        CGPoint delta = ccpSub(newCenterPointInParentNodeSpace, centerPointInParentNodeSpace);
		
        // offset contentNode in scrollableview space so centerpoint remains at the same location
        contentNode.position = ccpSub(contentNode.position, delta);
    }
...
-(void)loadLabel {
	positionLabel = [CCLabel labelWithString:@"" fontName:@"Arial" fontSize:20];
	positionLabel.color = ccBLACK;
	[self updateLabelText];
	[self addChild:positionLabel z:1];
	positionLabel.position = ccp(100, 100);	
}
-(void) updateLabelText {
	[positionLabel setString:[NSString stringWithFormat:@"%.0f, %.0f", contentNode.position.x, contentNode.position.y]];
}
